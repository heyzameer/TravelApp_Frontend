...
const BookingsList: React.FC = () => {
    ...
    const handleViewBooking = (id: string) => {
        // Navigate to booking details page (to be created or modal)
        // For now, simple console log or toast, or route if exists
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${statusFilter === 'all' ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                            >
                                All
                            </button>
                            <button
                                onClick={() => setStatusFilter('pending')}
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${statusFilter === 'pending' ? 'bg-yellow-500 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                            >
                                Pending
                            </button>
                            <button
                                onClick={() => setStatusFilter('confirmed')}
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${statusFilter === 'confirmed' ? 'bg-green-500 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                            >
                                Confirmed
                            </button>
                        </div>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredBookings.map((booking) => (
                                ...
                                >
                                    ...
                                    <td className="py-4 px-6">
                                        <span className="font-bold text-gray-800">₹{booking.totalAmount?.toFixed(2) || 0}</span>
                                    </td>
                                    <td className="py-4 px-6">
